...
def new_rand():
    ...
    while length > len(password):
        ...
        rand = random.randrange(0, len(char))#generowanie losowego znaku
        password += char[rand]#tworzenie hasla-dodawania losowego znaku
        if length == 1:#sprawdzanie czy haslo mac tylko jeden znak jesli tak, zakoncz proram
            break
        i += 1
        if len(password) == 1:
            rand2 = random.randrange(0, len(char))
            password += char[rand2]#dodawanie drugiego znaku (pierwszy w tablicy)(potrzebne do sprawdzenia czy się nie powtarza ze znakiem poprzednim)
...
received_pass = Entry(root, text="", font=("Verdana", 15), bd=0, bg="systembuttonface")
received_pass.pack(pady=20)
...
generate_button = Button(my_frame, borderwidth='0', text="Generuj", font=22, command=new_rand,
                         activebackground="green", width=7)
generate_button.grid(row=0, column=0)
generate_button.configure(bg="gray", font=("calibri", 12, "bold"))

copy_button = Button(my_frame, borderwidth='0', text="Skopiuj", font=22, command=copy, activebackground="#4169E5",
                     width=7)
copy_button.grid(row=0, column=1, padx=10)
copy_button.configure(bg="gray", font=("calibri", 12, "bold"))

save_button = Button(my_frame, borderwidth="0", text="Zapisz", font=22, command=save, activebackground="#4169E5",
                     width=7)
